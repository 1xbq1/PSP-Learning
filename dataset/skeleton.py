...
class Skeleton(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_numpy1 = np.concatenate((velocity, np.zeros((C, 1, V, M))), 1)
        ...
        if self.random_choose:
            ...
            data_numpy1 = random_sample_np(data_numpy1, self.window_size)
            # data_numpy = random_choose_simple(data_numpy, self.final_size)
        else:
            data_numpy = uniform_sample_np(data_numpy, self.window_size)
            data_numpy1 = uniform_sample_np(data_numpy1, self.window_size)
        if self.center_choose:
            ...
            data_numpy1 = random_choose_simple(data_numpy1, self.final_size, center=True)
        else:
            data_numpy = random_choose_simple(data_numpy, self.final_size)
            data_numpy1 = random_choose_simple(data_numpy1, self.final_size)

        if self.mode == 'train':
            return data_numpy.astype(np.float32), data_numpy1.astype(np.float32), label
        else:
            return data_numpy.astype(np.float32), data_numpy1.astype(np.float32), label, sample_name

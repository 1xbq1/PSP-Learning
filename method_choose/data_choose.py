...
def data_choose(args, block):
    if args.mode == 'test' or args.mode == 'watch_off':
        ...
        if args.data == 'ntu_skeleton':
            workers = args.worker
            label_data_set_train = NTU_SKE(mode='train', **args.data_param['label_train_data_param'])
            unlabel_data_set_train = NTU_SKE(mode='train', **args.data_param['unlabel_train_data_param'])
            ...
            raise (RuntimeError('No data loader'))
        data_loader_val = DataLoader(data_set_val, batch_size=args.test_batch_size, shuffle=False,
                                     num_workers=workers, drop_last=False, pin_memory=args.pin_memory,
                                     worker_init_fn=init_seed)
        label_data_loader_train = DataLoader(label_data_set_train, batch_size=args.label_train_batch_size, shuffle=True,
                                             num_workers=workers, drop_last=True, pin_memory=args.pin_memory,
                                             worker_init_fn=init_seed)
        unlabel_data_loader_train = DataLoader(unlabel_data_set_train, batch_size=args.unlabel_train_batch_size, shuffle=True,
                                               num_workers=workers, drop_last=True, pin_memory=args.pin_memory,
                                               worker_init_fn=init_seed)
        ...
        shutil.copy2(__file__, args.model_saved_name)
        return label_data_loader_train, unlabel_data_loader_train, data_loader_val
